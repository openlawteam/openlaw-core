...
class OpenlawVmSpec extends FlatSpec with Matchers {
  ...
  it should "being able to query the current state of the contract" in {
    ...
    vm.evaluate[OpenlawString]("Hello") shouldBe Right(OpenlawString("World"))
    vm.evaluate[OpenlawString]("Hello + ' World'") shouldBe Right(OpenlawString("World World"))
  }
  ...
  it should "be possible to register signature as incoming events and distinguish failed signatures" in {
    val template = "this is a contract [[Signatory:Identity]]"
		val Right(identity1Email) = Email("hello@world.com")
		val Right(identity2Email) = Email("wrong@gmail.com")
    val identity1: Identity = Identity.withEmail(identity1Email)
    val identity2: Identity = Identity.withEmail(identity2Email)
    val templateId = TemplateId(TestCryptoService.sha256(template))
    ...
    val definition1 = ContractDefinition(
      ...
      mainTemplate = templateId,
      templates = Map(),
      parameters = TemplateParameters(Map(VariableName("Signatory") -> IdentityType.internalFormat(identity1).getOrElse("")))
    )
  ...
  it should "be possible to sign the contract with an external signature" in {
    ...
    val definition = ContractDefinition(
      ...
      mainTemplate = templateId,
      templates = Map(),
      parameters = TemplateParameters(Map(VariableName("Signatory") -> ExternalSignatureType.internalFormat(ExternalSignature(serviceName = serviceName, identity = Some(identity))).getOrThrow()))
    )
  ...
  it should "be able to update the state and reflect this in the vm" in {
    ...
    val templateId = TemplateId(TestCryptoService.sha256(template))
    val Right(email) = Email("email@email.com")
    val Right(email2) = Email("email2@email.com")
    ...
    val definition = ContractDefinition(
      ...
      templates = Map(),
      parameters = TemplateParameters(
        "identity" -> IdentityType.internalFormat(identity).getOrElse(""),
        "identity2" -> IdentityType.internalFormat(identity2).getOrElse("")
      )
    )
    ...
    vm.allNextActions.map(_.size) shouldBe Right(1)

    vm.getAllExecutedVariables(EthereumCallType).size shouldBe 1

    vm.nextActionSchedule shouldBe Right(Some(startDate))

    val execution = EthereumSmartContractExecution(startDate, startDate, SuccessfulExecution, EthereumHash.empty)
    vm.allNextActions.map(_.size) shouldBe Right(1)

    val Right(action) = vm.allNextActions.map(_.head)
    vm.getAllExecutedVariables(EthereumCallType).size shouldBe 1
    vm.newExecution(action.identifier.getOrThrow(), execution)

    vm.nextActionSchedule.getOrThrow() shouldBe Some(LocalDateTime
      .now(clock)
      .withYear(2018).withMonth(12).withDayOfMonth(12).withHour(0).withMinute(1).withSecond(12).withNano(0)
    )

    vm.allNextActions.getOrThrow().size shouldBe 2
    vm.getAllExecutedVariables(EthereumCallType).size shouldBe 2

    vm.newExecution(action.identifier.getOrThrow(), EthereumSmartContractExecution(
  ...
  it should "be possible to register stopped contract, distinguish failed stops, and resume a stopped contract" in {
    val template = "this is a contract [[Signatory:Identity]]"
    val identity1: Identity = Identity.withEmail(Email("hello@world.com").getOrThrow())
    val identity2: Identity = Identity.withEmail(Email("wrong@gmail.com").getOrThrow())
    val templateId = TemplateId(TestCryptoService.sha256(template))
    val definition1 = ContractDefinition(
      creatorId = UserId.SYSTEM_ID,
      mainTemplate = templateId,
      templates = Map(),
      parameters = TemplateParameters("Signatory" -> IdentityType.internalFormat(identity1).getOrThrow())
    )
    val definition2 = ContractDefinition(
      creatorId = UserId.SYSTEM_ID,
      mainTemplate = templateId,
      templates = Map(),
      parameters = TemplateParameters("Signatory" -> IdentityType.internalFormat(identity2).getOrThrow())
    )
  ...
  it should "be able to send events to an event listener" in {
    ...
    val values = Map(VariableName("owner") -> EthAddressType.internalFormat(EthereumAddress("0x531E0957391dAbF46f8a9609d799fFD067bDbbC0").getOrThrow()).getOrThrow(), VariableName("value") -> NumberType.internalFormat(BigDecimal(2939)).getOrThrow())
    val identifier = vm.executionResult.map(_.allActions.getOrThrow()).getOrElse(Seq()).head.identifier.getOrThrow()
    val event = EthereumEventFilterEvent(identifier, EthereumHash.empty, EthereumAddress("0x531E0957391dAbF46f8a9609d799fFD067bDbbC0").getOrThrow(), "OpenlawSignatureEvent", values, LocalDateTime.now)
  ...
  it should "make general info about the contract available" in {
    ...
    val vm = vmProvider.create(definition, Some(EthereumAddress("0x531e0957391daff46f8a9609d799ffd067bdbbc0").getOrThrow()), OpenlawSignatureOracle(TestCryptoService, serverAccount.address), Seq())
    vm(LoadTemplate(template))
  ...
  it should "add init execution if we sign with ERC-712" in {
    ...
    val address = EthereumAddress("0x531E0957391dAbF46f8a9609d799fFD067bDbbC0").getOrThrow()
    val contractAddress = EthereumAddress("0x531e0957391dabf46f8a9609d799ffd067bdbbc0").getOrThrow()
    ...
    (for {
      ...
      _ <- vm.applyEvent(PreparedERC712SmartContractCallEvent(VariableName("Signature"), vm.executionResult.map(_.allActions.getOrThrow()).getOrElse(Seq()).head.identifier.getOrThrow(), address, EthereumSignature("0x123456789").getOrThrow(), contractAddress))
    } yield vm ) match {
      case Success(_) =>
        vm.initExecution[PreparedERC712SmartContractCallExecution](vm.executionResult.map(_.allActions.getOrThrow()).getOrElse(Seq()).head.identifier.getOrThrow()).getOrThrow() shouldBe Some(PreparedERC712SmartContractCallExecution(vm.executionResult.map(_.allActions.getOrThrow()).getOrElse(Seq()).head.identifier.getOrThrow(), EthereumSignature("0x1234567809").getOrThrow()))
      case Failure(ex, message) =>
        fail(message, ex)
    }
  }
  ...
  it should "not process events which do not match the filter" in {
    ...
    val values = Map(VariableName("owner") -> EthAddressType.internalFormat(EthereumAddress("0x531E0957391dAbF46f8a9609d799fFD067bDbbC0").getOrThrow()).getOrThrow(), VariableName("value") -> NumberType.internalFormat(BigDecimal(1000)).getOrThrow())
    val identifier = vm.executionResult.map(_.allActions.getOrThrow()).getOrElse(Seq()).head.identifier
    val event = EthereumEventFilterEvent(identifier.getOrThrow(), EthereumHash.empty, EthereumAddress("0x531E0957391dAbF46f8a9609d799fFD067bDbbC0").getOrThrow(), "OpenlawSignatureEvent", values, LocalDateTime.now)
