...
trait SignedActionOracle[R <: SignedActionEvent] extends OpenlawOracle[R] {
  ...
  def checkAction(
      ...
      event: R,
      actionData: EthereumData
  ): Result[OpenlawVm] = {
    val identityResult = vm
      .getAllVariables(IdentityType)
      .map {
        case (executionResult, variable) =>
          (variable.name, executionResult.evaluate[Identity](variable.name))
      }
      .collect {
    ...
    val externalSignatureResult = vm
      ...
      .map {
        case (executionResult, variable) =>
          (
            variable.name,
            executionResult.evaluate[ExternalSignature](variable.name)
          )
      }
      .collect {
    ...
    identityResult.flatMap {
      case Some(((name: VariableName, identity: Identity), _)) =>
        processEvent(vm, event, name.name, identity)
      case _ =>
        externalSignatureResult.flatMap {
          ...
          case _ =>
            Failure(
              "invalid event! no matching identity while resuming the contract"
            )
        }
    }
  }
