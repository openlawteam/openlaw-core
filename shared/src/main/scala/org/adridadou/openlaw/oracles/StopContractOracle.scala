    ...
    vm.getAllVariables(IdentityType)
      ...
      .find({
        case (_, Right(identity)) =>
          vm.isSignatureValid(vm.contractDefinition.id(crypto).stopContract(crypto), OpenlawSignatureEvent(vm.contractId, identity.userId, identity.email, "", event.signature, EthereumHash.empty))
        case _ => false
      }) match {
