        ...
        variable
          ...
          .map {
            case Success(Some(eventFilter)) =>
              ...
              val result = for {
                structureType <- generateStructureType(name, eventFilter.get, executionResult)
                ...
                (eventFilter.get.contractAddress.evaluate(child).map(_.get), eventFilter.get.eventType.evaluate(child).map(_.get), eventFilter.get.conditionalFilter.evaluate(child).map(_.get)) match {
