...
trait Expression {
  def missingInput(executionResult: TemplateExecutionResult): ResultNel[Unit]

  def validate(executionResult: TemplateExecutionResult): Result[Unit]

  def minus(right: Expression, executionResult: TemplateExecutionResult): Option[Result[Any]] =
    expressionType(executionResult).minus(evaluate(executionResult), right.evaluate(executionResult), executionResult)
  def plus(right: Expression, executionResult: TemplateExecutionResult): Option[Result[Any]] =
    expressionType(executionResult).plus(evaluate(executionResult), right.evaluate(executionResult), executionResult)
  def multiply(right: Expression, executionResult: TemplateExecutionResult): Option[Result[Any]] =
    expressionType(executionResult).multiply(evaluate(executionResult), right.evaluate(executionResult), executionResult)
  def divide(right: Expression, executionResult: TemplateExecutionResult): Option[Result[Any]] =
    expressionType(executionResult).divide(evaluate(executionResult), right.evaluate(executionResult), executionResult)

  def expressionType(executionResult: TemplateExecutionResult):VariableType
  def evaluate(executionResult: TemplateExecutionResult): Option[Result[Any]]
  def evaluateT[T](executionResult: TemplateExecutionResult)(implicit classTag:ClassTag[T]): Option[Result[T]] =
