...
case object EthAddressType extends VariableType("EthAddress") {
  ...
  ): Result[EthereumAddress] = EthereumAddress(value)

  override def internalFormat(value: OpenlawValue): Result[String] =
    VariableType.convert[EthereumAddress](value).map(_.withLeading0x)

  override def construct(
      constructorParams: Parameter,
      executionResult: TemplateExecutionResult
  ): Result[Option[EthereumAddress]] =
    for {
      ...
      result <- value match {
        ...
        case Some(value) =>
          Failure(
            "wrong type " + value.getClass.getSimpleName + ". expecting String"
          )
...
class EthereumAddress(val address: Array[Byte]) extends OpenlawNativeValue {
  ...
  override def equals(obj: scala.Any): Boolean = obj match {
    case EthereumAddress(otherAddress) =>
      util.Arrays.equals(otherAddress, address)
    case _ => false
  }
}
...
object EthereumAddress {
  ...
  def unapply(address: EthereumAddress): Option[Array[Byte]] =
    Some(address.address)

  def apply(address: Array[Byte]): Result[EthereumAddress] = {
    if (address.length > maxAddressSize) {
      Failure(
        "byte array of the address cannot be bigger than 20.value:" + bytes2hex(
          address
        )
      )
    } else {
  ...
  def apply(a: String): Result[EthereumAddress] = Option(a) match {
    case None => empty
    case Some(address) if address.startsWith("0x") =>
      apply(address.substring(2))
    case Some(address) if address.length =!= 40 =>
      Failure(
        s"the address string should be 40 or 42 with '0x' prefix. value(${address})"
      )
  ...
  implicit val ethAddressTypeEq: Eq[EthereumAddress] =
    (x: EthereumAddress, y: EthereumAddress) => x.equals(y)
}
...
final case class EthereumSignature(signature: Array[Byte])
  ...
  override def equals(obj: scala.Any): Boolean = obj match {
    case EthereumSignature(otherSignature) =>
      util.Arrays.equals(otherSignature, signature)
    case _ => false
  }
}
...
object EthereumSignature {
  ...
  implicit val ethereumSignatureEq: Eq[EthereumSignature] =
  ...
  def convert(arg: Any): Result[EthereumSignature] = arg match {
    ...
    case _ =>
      Failure(
        s"cannot convert type ${arg.getClass.getSimpleName} to EthereumSignature"
      )
  }
}
...
final case class EthereumData(data: Array[Byte]) extends OpenlawNativeValue {
  ...
  def merge(data: EthereumData): EthereumData =
    EthereumData(this.data ++ data.data)
}
...
object EthereumHash {
  ...
  def apply(hash: String): EthereumHash =
    new EthereumHash(EthereumAddress.hex2bytes(hash))
