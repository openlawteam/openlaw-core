...
object SignatureRSVParameter {
  implicit val signatureRSVParameterEnc: Encoder[SignatureRSVParameter] =
    deriveEncoder
  implicit val signatureRSVParameterDec: Decoder[SignatureRSVParameter] =
    deriveDecoder
}
...
final case class SignatureRSVParameter(
    rExpr: Expression,
    sExpr: Expression,
    vExpr: Expression
) {
  ...
  ): Result[Option[SignatureRSVParameterNames]] =
    (for {
...
case object EthereumCallType
    extends VariableType("EthereumCall")
    with ActionType {
  final case class EthereumCallPropertyDef(
      typeDef: VariableType,
      data: Seq[EthereumSmartContractExecution] => Option[OpenlawValue]
  )

  private val propertyDef: Map[String, EthereumCallPropertyDef] =
    Map[String, EthereumCallPropertyDef](
      "isSuccessful" -> EthereumCallPropertyDef(
        typeDef = YesNoType,
        _.headOption.map(_.executionStatus === SuccessfulExecution)
      ),
      "isFailure" -> EthereumCallPropertyDef(
        typeDef = YesNoType,
        _.headOption.map(_.executionStatus === FailedExecution)
      ),
      "executionDate" -> EthereumCallPropertyDef(
        typeDef = DateTimeType,
        _.headOption.map(_.executionDate)
      ),
      "tx" -> EthereumCallPropertyDef(
        typeDef = EthTxHashType,
        _.headOption.map(_.tx)
      )
    )
  ...
  override def cast(
      value: String,
      executionResult: TemplateExecutionResult
  ): Result[EthereumSmartContractCall] =
  ...
  private def checkIfHasProperty(key: VariableMemberKey): Result[Unit] =
    key match {
      case VariableMemberKey(Left(VariableName(k))) =>
        propertyDef
          ...
          .getOrElse(
            Failure(
              s"Ethereum Call type does not have the property $key defined"
            )
          )
      case _ =>
  ...
  override def construct(
      ...
      executionResult: TemplateExecutionResult
  ): Result[Option[EthereumSmartContractCall]] = {
    constructorParams match {
      case Parameters(v) =>
        ...
        for {
          ...
          optNetwork <- getParameter(values, "network")
            .map(getExpression)
            .sequence
            .map(_.getOrElse(NoopConstant(TextType)))
          signatureParameter <- getParameter(values, "Signature parameter")
            .map(getExpression)
            .sequence
          signatureRSVParameter <- getParameter(
            values,
            "Signature RSV parameter"
          ).map(getSignatureRSVParameter).sequence
          functionName <- getExpression(values, "function")
          expressionList <- getParameter(
            values,
            "arguments",
            "parameters",
            "params"
          ).map(getExpressionList).sequence.map(_.getOrElse(Seq()))
          startDate <- values
            .get("startDate")
      ...
      case _ =>
        Failure(
          "Ethereum Calls need to get 'contract', 'interface', 'network', 'function' as constructor parameter"
        )
    }
  }

  private def getSignatureRSVParameter(
      param: Parameter
  ): Result[SignatureRSVParameter] = param match {
  ...
  override def getTypeClass: Class[_ <: EthereumSmartContractCall] =
    classOf[EthereumSmartContractCall]

  def thisType: VariableType = EthereumCallType

  private def getExpressionList(param: Parameter): Result[Seq[Expression]] =
    param match {
      case ListParameter(exprs)    => Success(exprs)
      case OneValueParameter(expr) => Success(Seq(expr))
      case _ =>
        Failure(
          "invalid parameter type " + param.getClass.getSimpleName + " expecting list of expressions"
        )
    }
  ...
  override def actionValue(
    ...
    VariableType.convert[EthereumSmartContractCall](value)
}
