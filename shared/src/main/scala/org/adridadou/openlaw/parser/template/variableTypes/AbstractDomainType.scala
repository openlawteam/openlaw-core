...
final case class DomainInformation(
  ...
  def validate(
    ...
    executionResult
      .withVariable(VariableName("this"), value, typeDefinition)
      .toResultNel
...
case object AbstractDomainType
    extends VariableType(name = "DomainType")
    with TypeGenerator[DomainInformation] {
  ...
  private def getValue(
    ...
    values
      ...
      .map(Success(_))
      .getOrElse(
        Failure(s"missing property $propertyName in Domain Type constructor")
      )

  private def getVariableType(
      param: Parameter,
      executionResult: TemplateExecutionResult
  ): Result[VariableType] = param match {
    ...
    case OneValueParameter(definition: VariableDefinition) =>
      (for {
        ...
        .map(Success(_))
        .getOrElse(Failure(s"type ${definition.name} not found"))
    case _ =>
  ...
  override def construct(
      param: Parameter,
      executionResult: TemplateExecutionResult
  ): Result[Option[DomainInformation]] = param match {
    ...
    case _ =>
      Failure(
        """Domain type requires parameters (either 'variableType', 'condition', or 'errorMessage' is missing)"""
      )
  }
  ...
  override def cast(
      value: String,
      executionResult: TemplateExecutionResult
  ): Result[DomainInformation] =
    Failure("domain type definition cannot be casted")

  override def internalFormat(value: OpenlawValue): Result[String] =
    Failure("no internal format for domain type definition")
...
object DefinedDomainType {
  implicit val definedDomainTypeEnc: Encoder[DefinedDomainType] =
    (a: DefinedDomainType) => a.serialize
  implicit val definedDomainTypeDec: Decoder[DefinedDomainType] =
    (c: HCursor) =>
      for {
...
final case class DefinedDomainType(domain: DomainInformation, typeName: String)
  ...
  override def getTypeClass: Class[_ <: OpenlawValue] =
    domain.typeDefinition.getTypeClass
