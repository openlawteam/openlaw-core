  ...
  private def validate(
    ...
    (leftType, rightType) match {
      case (l, _) if l =!= YesNoType =>
        Failure(
          "The left part of the expression has to be a Boolean but instead is " + l.name + ":" + left.toString
        )
      case (_, r) if r =!= YesNoType =>
        Failure(
          "The right part of the expression has to be a Boolean but instead is " + r.name + ":" + right.toString
        )
      case _ =>
        Success(())
    }
...
final case class BooleanUnaryExpression(
    expr: Expression,
    op: BooleanUnaryOperation
) extends Expression {
  override def expressionType(
      executionResult: TemplateExecutionResult
  ): Result[VariableType] = Success(YesNoType)
    ...
    expr
      ...
      .flatMap { option =>
        option.map {
          ...
          case other =>
            Failure(
              s"bad type. Expected Boolean but got ${other.getClass.getSimpleName}:${expr.toString}"
            )
        }.sequence
      }
  ...
  override def validate(
    ...
    expr.expressionType(executionResult).flatMap { exprType =>
      if (exprType =!= YesNoType) {
        Failure(
          "The expression needs to be a Boolean but instead is " + exprType.name + ":" + expr.toString
        )
      } else {
        Success(())
      }
  ...
  override def variables(
      executionResult: TemplateExecutionResult
  ): Result[List[VariableName]] = expr.variables(executionResult)
