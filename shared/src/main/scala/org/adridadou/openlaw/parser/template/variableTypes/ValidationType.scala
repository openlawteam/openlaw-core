...
case object ValidationType
    extends VariableType(name = "Validation")
    with NoShowInForm {

  override def cast(
      value: String,
      executionResult: TemplateExecutionResult
  ): Result[Validation] = decode[Validation](value) match {
  ...
  def construct2(
      ...
      executionResult: TemplateExecutionResult
  ): Result[Validation] = constructorParams match {
    case Parameters(v) =>
      val values = v.toMap
      for {
    ...
    case _ =>
      Failure(
        "Validation need to get 'condition' and 'errorMessage' as constructor parameter"
      )
  }
  ...
  private def validate(
    ...
    for {
      ...
      errorMessageType <- validation.errorMessage.expressionType(
        executionResult
      )
      result <- {
        if (conditionType =!= YesNoType) {
          Failure(
            s"the condition expression of a validation needs to be of type YesNo, instead it is ${conditionType.name}"
          )
        } else if (errorMessageType =!= TextType) {
          Failure(
            s"The error message expression of a validation needs to be of type String, instead it is ${errorMessageType.name}"
          )
        } else {
          Success(validation)
        }
...
final case class Validation(condition: Expression, errorMessage: Expression)
    extends OpenlawNativeValue {
  def validate(executionResult: TemplateExecutionResult): ResultNel[Unit] =
    (for {
      value <- condition.evaluateT[OpenlawBoolean](executionResult)
      evaluatedErrorMessage <- errorMessage.evaluateT[OpenlawString](
        executionResult
      )
      _ <- value
        .filter(_ === false)
        .map(_ =>
          Failure(
            evaluatedErrorMessage.getOrElse(
              "validation error (error message could not be resolved)"
            )
          )
        )
        .getOrElse(Success(()))
