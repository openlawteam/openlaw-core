...
object EthereumSmartContractCall {
  implicit val smartContractEnc: Encoder[EthereumSmartContractCall] =
    deriveEncoder
  implicit val smartContractDec: Decoder[EthereumSmartContractCall] =
    deriveDecoder
}
...
final case class EthereumSmartContractCall(
  ...
  def isERC712Call: Boolean =
    signatureParameter.isDefined || signatureRSVParameter.isDefined

  def callKey(
      executionResult: TemplateExecutionResult,
      crypto: CryptoService
  ): Result[Option[EthereumData]] = {
    (for {
      fromResult <- from.flatMap(
        _.evaluate(executionResult)
          .flatMap(_.map(EthAddressType.convert).sequence)
          .sequence
      )
      toResult <- address
      ...
      for {
        ...
        ethereumData <- {
          val args = arguments.toList
            .map { expr =>
              val x = expr
                ...
                .flatMap(varType =>
                  expr
                    .evaluate(executionResult)
                    .flatMap(_.map(varType.internalFormat).sequence)
                )
              x
            }
            .sequence
            .map(_.flatten)
            .map { args =>
              val argHash = EthereumData(
                crypto.sha256(
                  args
                    .map(arg => EthereumData(crypto.sha256(arg)).withLeading0x)
                    .mkString("")
                )
              )
              EthereumData(
                crypto.sha256(
                  from.withLeading0x + to.withLeading0x + argHash.withLeading0x
                )
              )
            }
          args
      ...
      result <- {
        signatureParameterValue
          ...
          .flatMap {
            ...
            case None =>
              signatureRSVParameterValue
                .getRsv(executionResult)
                .map(_.map(rsv => List(rsv.r, rsv.s, rsv.v)))
          }
          .sequence
      }
...
final case class SmartContractMetadata(protocol: String, address: String)
