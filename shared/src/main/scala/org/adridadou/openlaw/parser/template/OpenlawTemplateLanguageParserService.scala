...
class OpenlawTemplateLanguageParserService(val internalClock: Clock) {
  ...
  private def redefine(redefinition: VariableRedefinition,
                       name: String,
                       varTypeDefinition: VariableTypeDefinition,
                       description: Option[String]): VariableRedefinition = {
  ...
  private def variableTypesMap(
    ...
    case ConditionalBlock(block, elseBlock, conditionalExpression) =>
      ...
      val newTypeMap2 = block.elems.foldLeft(newTypeMap)(
        (currentTypeMap, elem) =>
          currentTypeMap.flatMap(variableTypesMap(elem, _))
      )
      elseBlock
        .map(
          _.elems.foldLeft(newTypeMap2)(
            (currentTypeMap, elem) =>
              currentTypeMap.flatMap(variableTypesMap(elem, _))
          )
        )
    ...
    case ConditionalBlockSet(blocks) =>
      val initialValue: Result[VariableRedefinition] = Success(redefinition)
      blocks.foldLeft(initialValue)(
        (currentTypeMap, block) =>
  ...
  def forPreview(structuredAgreement: StructuredAgreement,
                 overriddenParagraphs: ParagraphEdits): String =
  ...
  def forPreview(structuredAgreement: StructuredAgreement,
                 ...
                 hiddenVariables: List[String]): String =
    XHtmlAgreementPrinter(
      preview = true,
  ...
  def forReview(structuredAgreement: StructuredAgreement,
                overriddenParagraphs: ParagraphEdits): String =
  ...
  def render[T](structuredAgreement: StructuredAgreement,
                overriddenParagraphs: ParagraphEdits,
                agreementPrinter: AgreementPrinter[T],
                hiddenVariables: Set[String]): Result[AgreementPrinter[T]] =
  ...
  def handleOverriddenParagraph[T](p: AgreementPrinter[T],
                                   str: String): Result[AgreementPrinter[T]] =
    MarkdownParser
      .parseMarkdown(str)
  ...
  private def renderElement[T](
    ...
    (element, optParagraph) match {
      ...
      case (variable: VariableElement, _)
          if variable.dependencies.forall(
            variable => !hiddenVariables.contains(variable)
          ) =>
        variable.content
