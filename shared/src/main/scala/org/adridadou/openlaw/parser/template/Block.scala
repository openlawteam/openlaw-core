...
final case class Block(elems: List[TemplatePart] = Nil) {
  ...
  def variableAliases(e: List[TemplatePart]): List[VariableAliasing] =
    e.flatMap({
      case variable: VariableAliasing => List(variable)
      case ConditionalBlock(block, elseBlock, _) =>
        block.variableAliases() ++ elseBlock
          .map(_.variableAliases())
          .getOrElse(Nil)
  ...
  private def variables(
    ...
    e.flatMap({
        case variable: VariableDefinition
            if variable.name.name.nonEmpty && !aliases
              .exists(_.name === variable.name) =>
          Some(variable)
        case ConditionalBlock(block, elseBlock, conditionalExpression) =>
          block.variables() ++ elseBlock
            .map(_.variables())
            .getOrElse(Nil) ++ expressionVariables(conditionalExpression)
        case ConditionalBlockSet(blocks) => variables(blocks, aliases)
        case ForEachBlock(_, expression, block) =>
          block.variables() ++ expressionVariables(expression)
  ...
  private def expressionVariables(
      expression: Expression
  ): List[VariableDefinition] = {
    expression match {
      case variable: VariableDefinition => List(variable)
      case ComparisonExpression(expr1, expr2, _) =>
        expressionVariables(expr1) ++ expressionVariables(expr2)
      case EqualsExpression(expr1, expr2) =>
        expressionVariables(expr1) ++ expressionVariables(expr2)
      case BooleanExpression(expr1, expr2, _) =>
        expressionVariables(expr1) ++ expressionVariables(expr2)
