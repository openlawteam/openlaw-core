...
object AbstractFunctionType
    extends VariableType(name = "Function")
  ...
  ): Result[OLFunction] = Failure("function type definition cannot be casted")

  override def internalFormat(value: OpenlawValue): Result[String] =
    Failure("no internal format for function type definition")
...
final case class FunctionType(typeParameter: VariableType)
    extends VariableType(name = typeParameter.name)
    with ParameterType {
  ...
  override def construct(
      param: Parameter,
      executionResult: TemplateExecutionResult
  ): Result[Option[OLFunction]] = param match {
    case OneValueParameter(function: OLFunction) =>
      Success(Some(function))
    case parameter =>
      Failure(
        s"function must have one function as constructor parameter, instead received ${parameter.getClass}"
      )
  }
  ...
  override def cast(
      value: String,
      executionResult: TemplateExecutionResult
  ): Result[OpenlawValue] = Failure("function type cannot cast")

  override def internalFormat(value: OpenlawValue): Result[String] =
    Failure("function type cannot format")
}
...
final case class OLFunctionCall(name: VariableName, parameter: Expression)
    extends Expression {
  override def missingInput(
    ...
    executionResult
      .getVariable(name)
      .map(_.evaluateT[OLFunction](executionResult))
      .sequence
      .map(_.flatten)
      .flatMap({
        case Some(func) =>
  ...
  override def validate(
      executionResult: TemplateExecutionResult
  ): Result[Unit] = executionResult.getVariable(name) match {
    case Some(variable) =>
      variable.varType(executionResult) match {
        ...
        case otherType =>
          Failure(
            s"${name.name} should be a function but instead is ${otherType.name}"
          )
      }
    case None =>
  ...
  override def expressionType(
    ...
    for {
      variable <- executionResult
        .getVariable(name)
        .toResult(s"function definition '${name.name}' cannot be found")
      func <- variable
        .evaluateT[OLFunction](executionResult)
        .flatMap(
          _.toResult(
            s"function definition missing for variable '${name.name}''"
          )
        )
      ier <- innerExecutionResult(func, executionResult)
  ...
  private def innerExecutionResult(
      func: OLFunction,
      executionResult: TemplateExecutionResult
    ...
    for {
      ...
      ier <- executionResult.withVariable(
        func.parameter.name,
        value,
        func.parameter.varType(executionResult)
      )
    } yield ier
...
final case class OLFunction(
  ...
  private def innerExecutionResult(executionResult: TemplateExecutionResult) =
    executionResult.withVariable(
      parameter.name,
      None,
      parameter.varType(executionResult)
    )
