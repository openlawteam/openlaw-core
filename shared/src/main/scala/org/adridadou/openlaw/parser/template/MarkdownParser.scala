...
object MarkdownParser {
  ...
  def createMarkdownParser(markdown: String): MarkdownParser =
    new MarkdownParser(markdown)

  def parseMarkdownOrThrow(markdown: String): List[TextElement] =
    parseMarkdown(markdown) match {
      case Success(seq) => seq
      case Failure(ex, message) =>
        throw new RuntimeException(
          s"error while parsing the markdown: $message"
        )
    }
...
class MarkdownParser(val input: ParserInput) extends Parser with MarkdownRules {
  ...
  def rootRule: Rule1[List[TextElement]] = rule {
    zeroOrMore(loosenTextElement) ~ EOI ~> (
        (elems: Seq[Seq[TextElement]]) => elems.flatten.toList
    )
  }
}
