...
import org.adridadou.openlaw.parser.template.{OneValueParameter, Parameter, TemplateExecutionResult, VariableMemberKey, VariableName}
...
object RegexType extends VariableType("Regex") with NoShowInForm {
  override def getTypeClass: Class[_ <: openlaw.OpenlawValue] = classOf[RegexValue]


  override def construct(constructorParams: Parameter, executionResult: TemplateExecutionResult): Result[Option[openlaw.OpenlawValue]] = {
    constructorParams match {
      case OneValueParameter(expr) =>
        for {
          paramType <- expr.expressionType(executionResult)
          _ <- if(paramType === TextType) Success.unit else Failure(s"parameter type should be text, instead it is ${paramType.name}")
  ...
  override def access(value: openlaw.OpenlawValue, variableName: VariableName, keys: List[VariableMemberKey], executionResult: TemplateExecutionResult): Result[Option[openlaw.OpenlawValue]] = {
    keys match {
      case Nil => Success(Some(value))
      case VariableMemberKey(Right(OLFunctionCall(VariableName("match"), parameter))) :: Nil =>
        for {
    ...
    keys match {
      ...
      case VariableMemberKey(Right(OLFunctionCall(VariableName("match"), parameter))) :: Nil =>
        for {
          ...
          _ <- if(expressionType === TextType) Success.unit else Failure(s"function 'match' takes a Text as parameter, not ${expressionType.name}")
        } yield ()
      case _ => super.validateKeys(variableName, keys, expression, executionResult)
    }
  }
  ...
  override def keysType(keys: List[VariableMemberKey], expression: Expression, executionResult: TemplateExecutionResult): Result[VariableType] = {
    keys match {
      ...
      case VariableMemberKey(Right(OLFunctionCall(VariableName("match"), _))) :: Nil => Success(YesNoType)
      case _ => super.keysType(keys, expression, executionResult)
    }
  }

  override def cast(value: String, executionResult: TemplateExecutionResult): Result[openlaw.OpenlawValue] =
    attempt(value.r).map(RegexValue)

  override def internalFormat(value: openlaw.OpenlawValue): Result[String] = value match {
    case RegexValue(value) => Success(value.toString())
    case other => Failure(s"should be Regex but instead was ${other.getClass.getSimpleName}")
  }
