...
import org.adridadou.openlaw.{BooleanOpenlawValue, OpenlawValue, SectionInfoOpenlawValue, StringOpenlawValue}
...
case object SectionType extends VariableType(name = "Section") with NoShowInForm {
  override type T = SectionInfoOpenlawValue
  ...
  override def cast(value: String, executionResult: TemplateExecutionResult): SectionInfoOpenlawValue = handleEither(decode[SectionInfo](value).map(SectionInfoOpenlawValue(_)))
  ...
  override def construct(constructorParams: Parameter, executionResult: TemplateExecutionResult): Result[Option[SectionInfoOpenlawValue]] = {
...
class SectionFormatter extends Formatter {
  override def format(value: OpenlawValue, executionResult: TemplateExecutionResult): Result[Seq[AgreementElement]] =
    attempt(VariableType.convert[SectionInfoOpenlawValue](value).get) map {
