...
final case class EventFilterDefinition(
    ...
    conditionalFilter: Expression
) extends ActionValue
    with OpenlawNativeValue {

  def abiEntries(
      executionResult: TemplateExecutionResult
  ): Result[List[AbiEntry]] =
    for {
      interfaceAny <- interface
        .evaluate(executionResult)
        .flatMap(_.toResult(s"expression '$interface' failed to evaluate"))
  ...
  def abiClasses(
    ...
    for {
      ...
      eventTypeAny <- eventType
        .evaluate(executionResult)
        .flatMap(_.toResult("could not evaluate eventType"))
  ...
  def findNameEntry(
    ...
    entries
      .collect {
        case entry @ AbiEntry(Some(name), _, _, _, _, _, _, _, _) =>
          (name, entry)
      }
  ...
  def convertVariablesOpenlawType(
      entry: AbiEntry
  ): Result[List[VariableDefinition]] =
  ...
  def convertVariableOpenlawType(param: AbiParam): Result[VariableDefinition] =
    attempt(AbiType.withName(param.`type`)).map { variableType =>
      VariableDefinition(
        ...
        Some(VariableTypeDefinition(variableType.openlawType.name))
      )
    }

  def convertVariableScalaTypes(param: AbiParam): Result[Class[_]] =
    attempt(AbiType.withName(param.`type`)).map { variableType =>
      variableType.scalaType
    }
