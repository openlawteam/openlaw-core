...
trait ExpressionRules extends JsonRules {
  ...
  def FunctionRule: Rule1[OLFunction] = rule {
    wsNoReturn ~ variableName ~ wsNoReturn ~ "=>" ~ wsNoReturn ~ ExpressionRule ~> (
        (
            name: VariableName,
            expression: Expression
        ) => OLFunction(VariableDefinition(name), expression)
    )
  }
  ...
  def BooleanTerm: Rule1[Expression] = rule {
    Term ~ wsNoReturn ~ zeroOrMore(BooleanPartial) ~> (
        (
            left: Expression,
            others: Seq[PartialOperation]
        ) =>
          others.foldLeft(left)({
            case (current, PartialOperation(op, right)) =>
              createOperation(current, op, right)
          })
      )
  }

  def BooleanPartial: Rule1[PartialOperation] = rule {
    booleanOperation ~ wsNoReturn ~ (Term | Factor) ~> (
        (
            op: String,
            expr: Expression
        ) => PartialOperation(op, expr)
    )
  }

  def ValuePartial: Rule1[PartialOperation] = rule {
    valueOperation ~ wsNoReturn ~ (SubTerm | Factor) ~> (
        (
            op: String,
            expr: Expression
        ) => PartialOperation(op, expr)
  ...
  def Term: Rule1[Expression] = rule {
    SubTerm ~ wsNoReturn ~ comparisonOperation ~ wsNoReturn ~ SubTerm ~> (
        (
            left: Expression,
            op: String,
            right: Expression
        ) => createOperation(left, op, right)
    )
  }

  def SubTerm: Rule1[Expression] = rule {
    Factor ~ wsNoReturn ~ zeroOrMore(ValuePartial) ~> (
        (
            left: Expression,
            others: Seq[PartialOperation]
        ) =>
          others.foldLeft(left)({
            case (left, PartialOperation(op, right)) =>
              createOperation(left, op, right)
          })
      ) |
      Factor
  ...
  def Factor: Rule1[Expression] = rule {
    constant | conditionalVariableDefinition | variableMemberInner | functionCall | variableNameOrBoolean | Parens | UnaryMinus | UnaryNot
  }
  ...
  def Parens: Rule1[Expression] = rule {
    '(' ~ wsNoReturn ~ ExpressionRule ~ wsNoReturn ~ ')' ~> (
        (expr: Expression) => ParensExpression(expr)
    )
  }

  def UnaryMinus: Rule1[Expression] = rule {
    '-' ~ wsNoReturn ~ ExpressionRule ~> (
        (expr: Expression) =>
          ValueExpression(NumberConstant(BigDecimal(-1)), expr, Multiple)
      )
  }

  def UnaryNot: Rule1[Expression] = rule {
    '!' ~ wsNoReturn ~ ExpressionRule ~> (
        (expr: Expression) => BooleanUnaryExpression(expr, Not)
    )
  }
  ...
  def variableAlias: Rule1[VariableAliasing] = rule {
    openS ~ wsNoReturn ~ variableAliasingDefinition ~ wsNoReturn ~ closeS
  }
  ...
  def functionCall: Rule1[OLFunctionCall] = rule {
    variableName ~ "(" ~ (FunctionRule | ExpressionRule) ~ ")" ~> (
        (
            name: VariableName,
            expression: Expression
        ) => OLFunctionCall(name, expression)
    )
  }
  ...
  def conditionalVariableDefinition: Rule1[VariableDefinition] = rule {
    variableName ~ ws ~ stringDefinition ~> (
        (
  ...
  def formatterDefinition: Rule1[FormatterDefinition] = rule {
    charsKeyAST ~ optional("(" ~ ws ~ parametersDefinition ~ ws ~ ")") ~> (
        (
            name: String,
            parameters: Option[Parameter]
        ) => FormatterDefinition(name, parameters)
    )
  }
    ...
    optional(capture("#")) ~
      ...
      (
          (
              ...
              name: VariableName,
              optVarType: Option[(VariableTypeDefinition, Option[Parameter])],
              formatter: Option[FormatterDefinition],
              desc: Option[String]
          ) => {
            val varTypeDefinition = optVarType.map({
              case (variableType, _) => variableType
            })
  ...
  def variableTypeDefinition
      : Rule1[(VariableTypeDefinition, Option[Parameter])] = rule {
    ":" ~ variableType ~ optional("(" ~ ws ~ parametersDefinition ~ ws ~ ")") ~> (
        (
  ...
  def variableType: Rule1[VariableTypeDefinition] = rule {
    capture(oneOrMore(keyChar)) ~ optional("<" ~ variableType ~ ">") ~> (
        (
            s: String,
            optTypeParameter: Option[VariableTypeDefinition]
        ) => VariableTypeDefinition(s.trim, optTypeParameter)
    )
  }
  ...
  def expression: Rule1[ExpressionElement] = rule {
    openS ~ ExpressionRule ~ wsNoReturn ~ optional(
      wsNoReturn ~ "|" ~ formatterDefinition
    ) ~ closeS ~> (
        (
            expr: Expression,
            formatter: Option[FormatterDefinition]
        ) => ExpressionElement(expr, formatter)
    )
  }

  def variableMember: Rule1[VariableMember] = rule {
    openS ~ wsNoReturn ~ variableMemberInner ~ optional(
      wsNoReturn ~ "|" ~ formatterDefinition
    ) ~ closeS ~> (
        (
            member: VariableMember,
            formatter: Option[FormatterDefinition]
        ) => member.copy(formatter = formatter)
    )
  }

  def variableMemberInner: Rule1[VariableMember] = rule {
    charsKeyAST ~ oneOrMore("." ~ variableMemberKey) ~> (
        (
            name: String,
            member: Seq[VariableMemberKey]
        ) => VariableMember(VariableName(name), member.toList, None)
    )
  }
  ...
  def parametersMapDefinition: Rule1[Parameters] = rule {
    oneOrMore(
      ws ~ charsKeyAST ~ ws ~ ":" ~ ws ~ (MappingParameterEntry | parameterEntry) ~ ws ~> (
          (
              key,
              value
          ) => key -> value
        ...
        (values: Seq[(String, Parameter)]) => Parameters(values.toList)
    )
  }

  def parameterEntry: Rule1[Parameter] = rule {
    oneOrMore(ExpressionRule).separatedBy(',') ~> (
        (n: Seq[Expression]) =>
          n.toList match {
  ...
  def MappingParameterEntry: Rule1[MappingParameter] = rule {
    oneOrMore(MappingRule)
      .separatedBy(',') ~> ((n: Seq[(VariableName, Expression)]) => {
  ...
  def MappingRule: Rule1[(VariableName, Expression)] = rule {
    ws ~ charsKeyAST ~ wsNoReturn ~ "->" ~ wsNoReturn ~ ExpressionRule ~ wsNoReturn ~> (
        (
            name: String,
            expr: Expression
        ) => (VariableName(name.trim), expr)
    )
  }

  def constant: Rule1[Expression] = rule {
    ws ~ (numberDefinition ~> (
        (constant: BigDecimal) => NumberConstant(constant)
    ) |
...
object Parameter {
  ...
  private def decodeParameter(
      name: String,
      cursor: HCursor
  ): Decoder.Result[Parameter] = {
    name match {
...
final case class Parameters(parameterMap: List[(String, Parameter)])
    extends Parameter {
  override def variables(
      executionResult: TemplateExecutionResult
  ): Result[List[VariableName]] =
    parameterMap
      .map { case (_, param) => param.variables(executionResult) }
      .sequence
...
final case class MappingParameter(mappingInternal: Map[String, Expression])
    extends Parameter {
  def mapping: Map[VariableName, Expression] =
    mappingInternal.map({ case (key, value) => VariableName(key) -> value })
  override def variables(
      executionResult: TemplateExecutionResult
  ): Result[List[VariableName]] =
...
object VariableTypeDefinition {
  implicit val variableTypeDefinitionEq: Eq[VariableTypeDefinition] =
    Eq.fromUniversalEquals
  implicit val variableTypeDefinitionEnc: Encoder[VariableTypeDefinition] =
    deriveEncoder
  implicit val variableTypeDefinitionDec: Decoder[VariableTypeDefinition] =
    deriveDecoder
}
