...
abstract class VariableType(val name: String) {
  ...
  def operationWith(rightType: VariableType,
                    operation: ValueOperation): VariableType =
    this
  ...
  def validateKeys(variableName: VariableName,
                   keys: List[VariableMemberKey],
                   expression: Expression,
                   executionResult: TemplateExecutionResult): Result[Unit] =
    keys.headOption
      .map(
        _ =>
          Failure(s"The variable $variableName of type $name has no properties")
  ...
  def keysType(keys: List[VariableMemberKey],
               expression: Expression,
               executionResult: TemplateExecutionResult): Result[VariableType] =
    if (keys.nonEmpty) {
      Failure(
  ...
  def combine[Y <: OpenlawValue](optLeft: Option[OpenlawValue],
                                 optRight: Option[OpenlawValue])(
  ...
  def isCompatibleType(otherType: VariableType,
                       operation: ValueOperation): Boolean =
    otherType === this

  def cast(value: String,
           executionResult: TemplateExecutionResult): Result[OpenlawValue]
  ...
  def getExpression(params: Map[String, Parameter],
                    names: String*): Result[Expression] =
  ...
  def getParameter(params: Map[String, Parameter],
                   names: String*): Option[Parameter] =
  ...
  def getMandatoryParameter(name: String,
                            parameter: Parameters): Result[Parameter] = {
...
object VariableType {
  ...
  def getPeriod(v: Expression,
                executionResult: TemplateExecutionResult): Result[Period] =
  ...
  def getString(v: Expression,
                executionResult: TemplateExecutionResult): Result[String] =
  ...
  private def createCustomType(cursor: HCursor,
                               name: String): Decoder.Result[VariableType] = {
