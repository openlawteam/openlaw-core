...
abstract class VariableType(val name: String) {
  ...
  def handleEither[T <: OpenlawValue](thisTry: Either[io.circe.Error, T]): T =
    thisTry match {
      case Right(v) => v
