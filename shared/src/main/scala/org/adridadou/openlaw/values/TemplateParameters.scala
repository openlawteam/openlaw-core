...
final case class TemplateParameters(params: Map[VariableName, String] = Map()) {
  def strParams: Map[String, String] =
    params.map({ case (key, value) => key.name -> value })
  ...
  def +(param: (VariableName, String)): TemplateParameters =
    this.copy(params + param)
  def ++(additionalParams: Map[VariableName, String]): TemplateParameters =
    this.copy(params ++ additionalParams)
}

object TemplateParameters {
  def apply(params: (String, String)*): TemplateParameters = {
    val names =
      params.map({ case (name, value) => VariableName(name) -> value })
    TemplateParameters(Map(names: _*))
  }

  implicit val templateParametersEnc: Encoder[TemplateParameters] =
    deriveEncoder
  implicit val templateParametersDec: Decoder[TemplateParameters] =
    deriveDecoder
}
