...
trait VariableExecutionEngine {
      ...
      case Some(definition: TemplateDefinition) =>
        Success(
          executionResult.copy(
            state = ExecutionWaitForTemplate(
              variable.name,
              definition.name,
              executionType
            )
  ...
  protected def addNewVariable(executionResult: OpenlawExecutionState,
                               variable: VariableDefinition): Unit =
    executionResult.variablesInternal append redefineDescription(
      executionResult,
      redefineType(executionResult, variable)
    )

  private def redefineType(executionResult: OpenlawExecutionState,
                           variable: VariableDefinition): VariableDefinition =
  ...
  private def defineAlias(executionResult: OpenlawExecutionState,
                          alias: VariableAliasing,
                          executed: Boolean): Result[OpenlawExecutionState] = {
  ...
  private def registerNewTypeIfNeeded(
    ...
    variable.varType(executionResult) match {
      case ChoiceType =>
        variable.defaultValue.map(
          param => ChoiceType.construct(param, executionResult)
        ) match {
      ...
      result <- {
        if (newType === oldType) {
          ...
          val unknownVariables = variables
            .filter(
              variable => executionResult.getVariable(variable.name).isEmpty
            )
  ...
  private def defineNewAlias(executionResult: OpenlawExecutionState,
                             alias: VariableAliasing,
                             executed: Boolean): Result[OpenlawExecutionState] =
    alias
      .variables(executionResult)
