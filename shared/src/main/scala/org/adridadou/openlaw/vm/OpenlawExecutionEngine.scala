...
class OpenlawExecutionEngine extends VariableExecutionEngine {
  ...
  /**
    * Entry point for simple agreements
    */
  def execute(mainTemplate: CompiledTemplate,
              parameters: TemplateParameters): Result[OpenlawExecutionState] =
    execute(
      mainTemplate,
      parameters,
      Map.empty,
  ...
  final def appendTemplateToExecutionResult(
    ...
    val newExecutionResult = executionResult.copy(
      ...
      sectionNameMappingInverseInternal = mutable.Map(
        executionResult.sectionNameMappingInverseInternal.toSeq: _*
      ),
  ...
  private def finishExecution(
    ...
    executionResult.parentExecutionInternal
      .map { parent =>
        (for {
          ...
          val result = if (!executionResult.embedded) {
            ...
            val variablesToAdd = executionResult.variablesInternal.filter(
              variable =>
            ...
            val executedVariablesToAdd =
              executionResult.executedVariablesInternal.filter(
                name =>
  ...
  private def processExecutedElement(
    ...
    case CodeBlock(elems) =>
      val initialValue: Result[OpenlawExecutionState] = Success(executionResult)
      elems.foldLeft(initialValue)(
        (exec, elem) => exec.flatMap(processCodeElement(_, templates, elem))
      )
    ...
    case section: VariableSection =>
      ...
      section.variables
        .foldLeft(initialValue)(
          (exec, variableDefinition) =>
            exec
              .flatMap(processVariable(_, variableDefinition, executed = true))
        )
      ...
      .flatMap {
        case (template, expressionType) =>
          ...
          foreachBlock.expression
            ...
            .map(
              vars => executionResult.executedVariablesInternal appendAll vars
            )
          ...
          elementsResult.flatMap { elements =>
            elements.foldLeft(initialValue)(
              (eitherExecutionResult, element) =>
  ...
  private def getRoot(parent: OpenlawExecutionState): OpenlawExecutionState =
    parent.parentExecutionInternal match {
  ...
  private def validateSubExecution(
    ...
    result.getVariables
      .foldLeft(initialValue)(
        (value, variable) =>
          value.flatMap(
            _ => validateSubExecution(result, templateDefinition, variable)
        )
      )
  }
  ...
  private def validateSubExecution(
    ...
    if ((otherType =!= variable.varType(result) && !variable.name.name
      ...
      result.parentExecutionInternal
        .map(
          parent =>
  ...
  private def processCodeElement(
    ...
    case section: VariableSection =>
      val initialValue: Result[OpenlawExecutionState] = Success(executionResult)
      section.variables.foldLeft(initialValue)(
        (exec, variableDefinition) =>
          exec.flatMap(
            e => processVariable(e, variableDefinition, executed = false)
        )
      )
    ...
    expression
      ...
      .flatMap {
        case Some(true) =>
          val initialValue: Result[OpenlawExecutionState] =
            Success(executionResult)
          val initialValue2 = block.elems.foldLeft(initialValue)(
            (exec, elem) => exec.flatMap(processCodeElement(_, templates, elem))
          )
          elseBlock
            .map(
              _.elems.foldLeft(initialValue2)(
                (exec, elem) =>
